...
export default function ConnectionHubScreen() {
  ...
  // State management
  const [connection, dispatch] = useReducer(connectionReducer, null, getInitialConnection);
  const [currentRole, setCurrentRole] = useState<Role>('HOST');
  const [activeTab, setActiveTab] = useState<TabKey>('shared');
  const [refreshing, setRefreshing] = useState(false);
  
  // Modal state
  const [resourceModalObligation, setResourceModalObligation] = useState<string | null>(null);
  const [consentModalObligation, setConsentModalObligation] = useState<string | null>(null);

  // Derived data
  const currentParty = currentRole === 'HOST' ? connection.host : connection.guest;
  const partnerParty = currentRole === 'HOST' ? connection.guest : connection.host;
  ...
  const getFilteredObligations = (): Obligation[] => {
    if (activeTab === 'shared') {
      ...
      return connection.obligations.filter(o => o.providerRole === currentRole);
    } else {
  ...
  const getProviderResources = (obligation: Obligation) => {
    const provider = obligation.providerRole === 'HOST' ? connection.host : connection.guest;
  ...
  const handleSelectResource = useCallback((obligationId: string, resourceId: string) => {
  ...
  const tabs = [
    ...
    { key: 'received', label: `${partnerParty.organization.name}'s Data` },
  ];

  const filteredObligations = getFilteredObligations();
  const allResources = [...currentParty.locker.resources, ...partnerParty.locker.resources];
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      {resourceObligation && (
        <ResourceModal
          ...
          onSelect={(resourceId) => handleSelectResource(resourceObligation.id, resourceId)}
      ...
      {consentObligation && (
        <ConsentModal
          ...
          currentRole={currentRole}
          canDecide={consentObligation.status === 'Fulfilled' && connection.status === 'Established'}
